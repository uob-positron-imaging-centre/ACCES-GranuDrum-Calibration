...
files = [f for f in glob("post/particles_*.vtk") if "boundingBox" not in f]
files = sorted(files, key=extract_timestep)

if len(files) == 0:
    raise FileNotFoundError("No VTK files found - did you run the simulation?")
...
image_path = "Measurements/gd_nature_45rpm_averaged.bmp"
...
print(f"Error to experiment = {error} mm2")
